    ...
    >
      ...
      {showLocate && (
        <ControlGroup>
          ...
          >
            {waitingForLocation ? <Spinner className="size-4" /> : <Locate className="size-4" />}
          </ControlButton>
        </ControlGroup>
      )}
      {showFullscreen && (
...
type MapRouteProps = {
  ...
  /** Line color as CSS color value (default: primary theme color) */
  color?: string;
...
function MapRoute({
  ...
  const color = colorProp ?? getCssVar("--primary", "#4285F4");
  const autoId = useId();
...
type MapClusterLayerProps<P extends GeoJSON.GeoJsonProperties = GeoJSON.GeoJsonProperties> = {
  ...
  /** Color for unclustered individual points (default: primary theme color) */
  pointColor?: string;
...
function MapClusterLayer<P extends GeoJSON.GeoJsonProperties = GeoJSON.GeoJsonProperties>({
  ...
  const pointColor = pointColorProp ?? getCssVar("--primary", "#3b82f6");
  const { map, isLoaded } = useMap();
  const id = useId();
